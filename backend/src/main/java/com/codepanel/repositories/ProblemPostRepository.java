...
public interface ProblemPostRepository extends JpaRepository<ProblemPost, UUID> {
    ...
    @Query("SELECT COUNT(s) FROM ProblemPostComment s WHERE s.problemPost.id = :problemPostId")
    Long countCommentsByProblemPostId(@Param("problemPostId") UUID problemPostId);
}
